...
def run_revision_agent(db: Session, *, review: Review, triggered_by: User) -> RevisionAgentResult:
    ...
    # Project Engineering Setup rule 6, extended to this path too — a
    # reviewer-requested revision is a real IMPROVE run, not a lesser one,
    # and previously got no coding-standards/guardrails/stack context at
    # all (only app/api/routes/agent_runs.py's generic Draft/Improve/
    # Validate action wired this in). A project with no engineering setup
    # is untouched (rule 10).
