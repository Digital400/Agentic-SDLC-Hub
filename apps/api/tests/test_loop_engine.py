...
`generate` and `validate_draft_quality` are monkeypatched so each test
controls exactly what a "model call" and a "validation pass" return,
independent of the mock/real AI provider and independent of
loop_validation.py's actual scoring heuristic — what's under test here is
...
from app.services.loop_validation import ValidationIssue, ValidationResult
...
def test_loop_stops_after_one_iteration_when_quality_meets_threshold(db, project, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.loop_engine.validate_draft_quality",
        lambda **kwargs: ValidationResult(quality_score=0.9, issues=[]),
    )
    ...
    result = engine.run_loop(
        ...
        approved_inputs={}, freeform_context={}, max_iterations=3, quality_threshold=0.8,
    )
    db.flush()
...
def test_loop_stops_at_max_iterations_when_quality_never_met(db, project, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: ValidationResult(
            quality_score=0.3, issues=[ValidationIssue("critical", "Still missing key content.")]
        ),
    )
...
def test_loop_stops_early_when_below_threshold_but_no_critical_issues(db, project, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.loop_engine.validate_draft_quality",
        lambda **kwargs: ValidationResult(
            quality_score=0.5, issues=[ValidationIssue("minor", "Could use a bit more detail.")]
        ),
    )
    ...
    result = engine.run_loop(
        run=run, project=project, node=node, action=AgentPromptRole.DRAFT, active_prompt=prompt,
        approved_inputs={}, freeform_context={}, max_iterations=5, quality_threshold=0.8,
    )
    db.flush()
...
def test_loop_stops_immediately_when_draft_needs_clarification(db, project, monkeypatch):
    ...
    def fake_validate(**kwargs):
        validate_calls["n"] += 1
        return ValidationResult(quality_score=1.0, issues=[])

    monkeypatch.setattr("app.services.loop_engine.validate_draft_quality", fake_validate)
...
# --- Improvement history: score should be able to rise across iterations ----------
...
def test_loop_records_improving_scores_across_iterations_as_improvement_history(db, project, monkeypatch):
    ...
    def fake_generate(**kwargs):
        call_count["n"] += 1
        # Each iteration's feedback should be threaded back in as
        # validation_feedback — the actual mechanism improvement acts on.
        if call_count["n"] > 1:
            assert kwargs["validation_feedback"] == ["Missing an overview section."]
        return _draft_result(content=f"draft v{call_count['n']}")
    ...
    def fake_validate(**kwargs):
        score = next(scores)
        issues = [] if score >= 0.8 else [ValidationIssue("critical", "Missing an overview section.")]
        return ValidationResult(quality_score=score, issues=issues)
